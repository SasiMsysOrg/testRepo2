...
const GetDates = (startDate, endDate) => {
  ...
  while (newStartDate < moment(endDate)) {
    if (WEEKENDS.indexOf(newStartDate.format('dddd')) < 0) {
      dates.push({ datetime: newStartDate.format('YYYY-MM-DDTHH:mm:ss[Z]'), dateOnly: newStartDate.format('YYYY-MM-DD') });
    }
  ...
  try {
    switch (true) {
      case type.toUpperCase().includes('HOURS'):
        return Math.abs(new Date(until).getTime() - new Date(since).getTime()) / 3600000;
      default:
